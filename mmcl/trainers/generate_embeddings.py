...
def generate_embeddings(hparams):
  ...
  if hparams.datatype == 'imaging' or hparams.datatype == 'multimodal':
    train_dataset = EvalImageDataset(hparams.data_train_eval_imaging, hparams.labels_train_eval_imaging, hparams.delete_segmentation, hparams.eval_train_augment_rate, hparams.img_size, train=False)
    val_dataset = EvalImageDataset(hparams.data_val_eval_imaging, hparams.labels_val_eval_imaging, hparams.delete_segmentation, hparams.eval_train_augment_rate, hparams.img_size, train=False)

    model = ResnetEmbeddingModel(hparams)
  elif hparams.datatype == 'tabular':
    train_dataset = EvalTabularDataset(hparams.data_train_eval_tabular, hparams.labels_train_eval_tabular)
    val_dataset = EvalTabularDataset(hparams.data_val_eval_tabular, hparams.labels_val_eval_tabular)
