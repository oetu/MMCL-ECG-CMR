...
def test(hparams, wandb_logger=None):
  ...
  if hparams.datatype == 'imaging' or hparams.datatype == 'multimodal':
    test_dataset = EvalImageDataset(hparams.data_test_eval_imaging, hparams.labels_test_eval_imaging, hparams.delete_segmentation, 0, grab_arg_from_checkpoint(hparams.checkpoint, 'img_size'), target=hparams.target, train=False, live_loading=hparams.live_loading)
    
    print(test_dataset.transform_val.__repr__())
  elif hparams.datatype == 'ecg':
    test_dataset = EvalECGDataset(hparams.data_test_eval_ecg, hparams.labels_test_eval_ecg)
